...
export const complainService = async (bookingId, userId, body) => {
  const { complaint } = body;
  const booking = await Booking.findById(bookingId).populate('serviceId');

  if (!booking) throw new Error('Booking not found');
  if (booking.clientId.toString() !== userId) throw new Error('Only clients can submit complaints');

  // Notify all admins about the complaint
  const admins = await User.find({ role: 'admin' });
  for (const admin of admins) {
    await Notification.create({
      ...
      message: `A client has submitted a complaint about booking #${booking._id.toString().slice(-6)} (${booking.serviceId?.title || 'Service'}): "${complaint}"`,
      bookingId: booking._id,
    });
  }

  return { success: true, message: 'Complaint submitted to admin' };
};
