                    ...
                    <div className="flex items-center gap-4">
                        {user ? (
                                    ...
                                    >
                                        ...
                                        <div className="text-left hidden sm:block pr-1">
                                            ...
                                            <div className="flex items-center gap-1 mt-1">
                                               ...
                                               <p className="text-[8px] font-black text-slate-400 uppercase tracking-[0.2em] leading-none">Online</p>
                                            </div>
                                        </div>
                                        ...
                                        <>
                                            ...
                                            <div className="absolute top-[130%] right-0 w-72 bg-white rounded-[32px] shadow-2xl border border-slate-100 py-4 z-20 animate-in fade-in zoom-in-95 duration-200 origin-top-right backdrop-blur-3xl overflow-hidden ring-1 ring-black/5">
                                                <div className="px-6 py-5 border-b border-slate-50 mb-3 bg-slate-50/50">
                                                    <p className="text-[9px] font-black text-slate-400 uppercase tracking-[0.2em] mb-1.5">Account</p>
                                                    <p className="text-sm font-black text-slate-900 truncate tracking-tight">{user.name}</p>
                                                    <p className="text-[9px] font-bold text-blue-500 uppercase tracking-widest mt-1">Logged In</p>
                                                </div>
                                                ...
                                                <div className="px-3 mt-3 pt-3 border-t border-slate-50">
                                                    ...
                                                    >
                                                        ...
                                                        <span className="text-xs font-black uppercase tracking-widest flex-1">Sign Out</span>
                                                    </button>
                                                </div>
                                            </div>
                                        </>
                            ...
                            <div className="flex items-center gap-6">
                                <Link to="/login" className="hidden sm:block text-[11px] font-black uppercase tracking-[0.2em] text-slate-900 hover:text-blue-600 transition-colors px-2">Log In</Link>
                                <Link to="/register">
                                    <Button className="rounded-2xl px-10 h-14 font-black text-[11px] uppercase tracking-widest shadow-2xl shadow-blue-500/20 active:scale-95 transition-all">Sign Up</Button>
                                </Link>
                            </div>
                        )}
                        ...
                        <nav className="flex flex-col gap-10 text-center">
                            ...
                            {!user && (
                                <div className="flex flex-col gap-5 mt-12 bg-slate-50 p-8 rounded-[40px] border border-slate-100">
                                    <Link onClick={() => setIsMenuOpen(false)} to="/login" className="text-xl font-black text-slate-900 uppercase tracking-widest">Log In</Link>
                                    <Link onClick={() => setIsMenuOpen(false)} to="/register">
                                        <Button className="w-full rounded-[24px] h-20 text-xl font-black uppercase tracking-widest shadow-2xl shadow-blue-500/20">Sign Up</Button>
                                    </Link>
                                </div>
                            )}
                            {user && (
                                <div className="flex flex-col gap-6 mt-12 pt-12 border-t border-slate-100">
                                    <Link onClick={() => setIsMenuOpen(false)} to={`/${user.role}/dashboard`} className="text-3xl font-black text-blue-600 tracking-tighter">Dashboard</Link>
                                    <button onClick={() => { setIsMenuOpen(false); setIsProfileModalOpen(true); }} className="text-xl font-black uppercase tracking-widest text-slate-900">Profile</button>
