...
const DashboardOverview = () => {
    ...
    const totalProviderEarnings = bookings.filter(b => b.status === 'Completed').reduce((acc, b) => acc + (b.commissionProvider || ((b.finalPrice || b.basePrice || 0) * 0.7)), 0);
    ...
    const stats = user?.role === 'provider' ? [
        ...
        { label: 'Total Spent', value: `Rs. ${bookings.reduce((acc, b) => acc + (b.finalPrice || b.basePrice || 0), 0).toLocaleString()}`, color: 'text-indigo-600', bg: 'bg-indigo-50', icon: Target }
    ];
                    ...
                    <div className="bg-white rounded-[42px] p-10 flex flex-col lg:flex-row items-center gap-10 border border-slate-100 relative overflow-hidden">
                        ...
                        <div className="flex-1 w-full grid grid-cols-1 md:grid-cols-2 gap-6">
                            {(() => {
                                ...
                                const displayPrice = latestUnpaid.finalPrice || latestUnpaid.basePrice || 0;
                                return (
                                    <>
                        ...
                        <div className="relative z-10 h-full flex flex-col">
                            ...
                            <div className="flex-1 flex flex-col justify-center gap-6">
                                <div>
                                    ...
                                    <h2 className={`text-5xl font-black tracking-tighter ${isClient ? 'text-slate-900' : 'text-white'}`}>Rs. {user?.role === 'provider' ? totalProviderEarnings.toLocaleString() : bookings.reduce((acc, b) => acc + (b.finalPrice || b.basePrice || 0), 0).toLocaleString()}</h2>
                                </div>
